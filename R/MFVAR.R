...
function(monthly, quarterly, p=3, prior="default") {
  ...
  # Length of quarterly and monthly data 
  nq <- NCOL(quarterly)
  nm <- NCOL(monthly)
  ...
  xq <- ts(matrix(rep(quarterly,each=3),NROW(quarterly)*3,nq), 
           frequency = 12, 
           start = month.start)
  xm <- monthly
  ...
  # Insert zeros for unobserved data
  ...
  L_qz <- matrix(rep(cbind(matrix(0,nq,nm),diag(1/3,nq)),3),nq,nm*3+nq*3)
  L_mz <- cbind(diag(nm),matrix(0,nm,nm*2+nq*3))
  L_z  <- cbind(matrix(0,nq+nm,1), rbind(L_mz,L_qz))
  
  if(p > 3) L_z <- cbind(L_z,matrix(0,nq+nm,(nq+nm)*(p-3)))

  ML_z <- array(L_z, dim=c(dim(L_z),nrow(x))) 

  na.index.xm <- which(is.na(apply(x[,1:nm,drop=FALSE],1,prod)))
  na.index.xq <- which(is.na(apply(x[,(nm+1):(nm+nq),drop=FALSE],1,prod)))
  
  #Replace all NAs with zero
  x[is.na(x)] <- 0

  # Replace transform on quarterly variables with zero where data unobserved
  for(i in na.index.xq) {
    if(i%%3 == 0)
      ML_z[nm+which(!(abs(x[i,(nm+1):(nm+nq)])>0)),,i] <- 0
    else
      ML_z[(nm+1):(nm+nq),,i] <- 0
  }
  # Replace transform on monthly variables with zero where data unobserved
  if(length(na.index.xm)>0) 
    for(i in na.index.xm) ML_z[1:nm,(1+1):(nm+1),i] <- diag(abs(x[i,1:nm])>0)*1
  ...
  # Constant and number of deterministic variables
  ...
  if(any(prior$prior=="mn")) {
    m1 <- diag(sigma)
    m2 <- matrix(0,K*(p-1),K)
    m3 <- if(M==0 || any(prior$prior=="ss")) NULL else matrix(0,M,K)
    m4 <- if(M==0 || any(prior$prior=="ss")) NULL else matrix(0,K*p,M)
    m5 <- if(M==0 || any(prior$prior=="ss")) NULL else t(m3)
    m6 <- if(M==0 || any(prior$prior=="ss")) NULL else diag(M)
    m7 <- if(M==0 || any(prior$prior=="ss")) NULL else t(m4)
    m8 <- diag(1:p)
    m9 <- matrix(0,K,K*p)
    YD <- rbind(prior$hp$lambda*m1*(1/prior$hp$tau), m2, m1, m3)
    ZD <- cbind(if(M==0 || any(prior$prior=="ss")) NULL 
                else rbind(m4, m5, m6/prior$hp$kappa),
                rbind((m8^prior$hp$rho)%x%m1*(1/prior$hp$tau), m9, m7))
  }
  # artificial data for sum of coefficients prior
  if(any(prior$prior=="sc")) {
    m1 <- diag(mu)
    m2 <- if(M==0) NULL else matrix(0,K,M)
  ...
  if(any(prior$prior=="st")) {
    ...
    m1 <- if(type=="const") c(1,rep(0,M-1)) else rep(0,M)
